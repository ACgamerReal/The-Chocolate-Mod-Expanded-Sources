...
public class EnderBarItem extends Item {



    public EnderBarItem(Settings settings) {
        super(settings);
    }
    ...
    public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
        ...
        if (!world.isClient) {
            ...
            double f = user.getZ();
            for (int i = 0; i < 16; ++i) {
                double g = user.getX() + (user.getRandom().nextDouble() - 0.5) * 8.0;
                ...
                double j = user.getZ() + (user.getRandom().nextDouble() - 0.5) * 8.0;
                if (user.hasVehicle()) {
                    user.stopRiding();
                }
